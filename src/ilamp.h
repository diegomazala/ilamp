...
public:
	...
	void build_kdtree(uint16_t kdtree_count, uint16_t num_neighbours = 4, uint16_t knn_search_checks = 128);

	Eigen::Matrix<Type, Eigen::Dynamic, Eigen::Dynamic> execute(Type x, Type y);
...
template <typename Type>
void ILamp<Type>::build_kdtree(uint16_t kdtree_count, uint16_t _num_neighbours = 4, uint16_t _knn_search_checks = 128)
{
...
ILamp<Type>::execute(Type px, Type py)
{
	try
	{
