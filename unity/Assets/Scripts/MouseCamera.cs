...
public class MouseCamera : MonoBehaviour 
{
    ...
    public bool cameraKeyOn = true;

    public void Start () 
    {
        if (pivotTransform)
            distance = Vector3.Distance(this.transform.position, pivotTransform.position);
		...
		GameObject go = new GameObject("_MouseCameraTarget");
	...
	public void Update () 
    {
        ...
        if (cam == null)
            cam = Camera.main;
		inputBounds.x = cam.pixelWidth * paramInputBounds.x;
		inputBounds.y = cam.pixelHeight * paramInputBounds.y;
		inputBounds.width = cam.pixelWidth * paramInputBounds.width;
		inputBounds.height = cam.pixelHeight * paramInputBounds.height;
	    ...
	    if(target && inputBounds.Contains(Input.mousePosition)) {
			...
			if( skyInput ) 
			{
                ...
                RenderSettings.skybox.SetFloat("_Rotation", dx);
			}
			else if( panInput ) {
